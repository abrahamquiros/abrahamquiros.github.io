...
public class Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "crearejercicio":
			...
			break;
		case "exportar":
			exportarEnternos(request, response);
			break;
		case "iraregistro":
	...
	private void crearHTML(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		String usuario=request.getParameter("usuario");
		AccesoDatos.crearHTML(usuario);
	}

	private void exportarEnternos(HttpServletRequest request, HttpServletResponse response) {
		// TODO Auto-generated method stub
		String usuario=request.getParameter("usuario");
	...
	private void crearEntreno(HttpServletRequest request, HttpServletResponse response) {
		...
		for(int i=1; i<5; i++) 
		{
			// Voy a ir recuperando parametros ejercicio_x y observaciones_x
	...
	private void gestionarLogin(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(rol==null) 
		{
			//Le mandamos a index.jsp
			request.getRequestDispatcher("index.jsp").forward(request, response);
			
		}
		if(rol.equals("admin")) 
		{
			//Le mandamos a creaejercicio.jsp
			request.setAttribute("master", "OK");
			request.getRequestDispatcher("crearejercicio.jsp").forward(request, response);
		}
		else if(rol.equals("user")) 
		{
			//Le mandamos a crearentreno.jsp con los atributos usuario y lista_ejercicios
			ArrayList<Ejercicio> lista_ejercicios=AccesoDatos.getExercices();
			request.setAttribute("lista_ejercicios", lista_ejercicios);
			request.setAttribute("usuario", usuario);
